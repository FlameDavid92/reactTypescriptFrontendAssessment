...
const Models: FC = () => {
  ...
  const [localTotal, setLocalTotal] = useState<Price>({value:0, currency:'$'});
  ...
  useEffect(() => {
    ...
    if(sel){
      ...
      setLocalTotal(parsed.total);
    }
  }, []);
  
  const handleProductChange = useCallback((startingPrice: Price, id:number) => {
    setLocalTotal(startingPrice);
