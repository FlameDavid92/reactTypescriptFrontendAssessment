...
const Colors: FC = () => {
  ...
  const handleColorChange = (ind: number, price: Price) => {
    ...
    localStorage.setItem("colors", JSON.stringify({selectedColor: selectedColor}));
  };
      ...
      <div className="col-12">
        {selectedProduct?.colors.map((c, ind) => {
          return (<span className="p-2" key={c.color}>
            <input type="radio" value={c.color} name="product" onChange={() => handleColorChange(ind, c.price)} />{" " +c.color+" "+c.price.currency+c.price.value}
          </span>
          )
        })}
      </div>
