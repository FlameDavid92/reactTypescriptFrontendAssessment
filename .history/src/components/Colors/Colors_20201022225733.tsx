...
const Colors: FC = () => {
    ...
    if(firstRender){
      ...
      if (sel) {
        ...
        if(col){
          ...
          selectedProduct && console.log(selectedProduct.id, parsed.currentProductId);
          if(selectedProduct && selectedProduct.id === parsed.currentProductId){
  ...
  const handleColorChange = (ind: number, price: Price) => {
    ...
    localStorage.setItem("colors", JSON.stringify({selectedColor: selectedColor, currentProductId: selectedProduct?.id}));
  };
