        ...
        <div className="row justify-content-center">
          {selectedProduct?.colors.map((c, ind) => {
            return (<div className="col-12 col-lg-3 mb-2" key={c.color}>
              <input type="radio" value={c.color} name="product" checked={ind === selectedColor} onChange={() => handleColorChange(ind, c.price)} /><span className="pl-3 myLabel">{c.color + " " + c.price.currency + c.price.value}</span>
            </div>
            )
          })}
        </div>
